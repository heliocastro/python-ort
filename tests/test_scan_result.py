...
def _make_provenance_dict() -> dict:
    """Create a provenance dict matching the YAML structure."""
    return {
        "vcs_info": {
            "type": "Git",
...
def test_scan_result_valid_minimal():
    ...
    result = ScanResult(
        provenance=_make_provenance_dict(),
...
def test_scan_result_provenance_with_vcs_info():
    ...
    assert isinstance(result.provenance, RepositoryProvenance)
